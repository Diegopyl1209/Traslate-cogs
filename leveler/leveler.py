...
try:
    ...
    raise RuntimeError(f"No se puede cargar pymongo/motor:{e}\nInstala los paquetes 'pymongo' y 'motor'")
try:
    import numpy
...
class Leveler(commands.Cog):
    ...
    async def profile(self, ctx, *, user: discord.Member = None):
        ...
        if await self.config.guild(ctx.guild).disabled():
            await ctx.send("**Los comandos de niveles para este servidor están deshabilitados!**")
            return
        ...
        if await self.config.guild(ctx.guild).text_only():
            ...
            async with ctx.channel.typing():
                ...
                await channel.send(
                    "**Perfil de usuario para {}**".format(await self._is_mention(user)),
                    file=file,
                )
    ...
    async def profile_text(self, user, server, userinfo):
        ...
        em.add_field(name="Titulo:", value=userinfo["title"] or None)
        em.add_field(name="Reps:", value=userinfo["rep"])
        em.add_field(name="Rango Global:", value="#{}".format(await self._find_global_rank(user)))
        em.add_field(
            name="Rango del servidor:",
        ...
        em.set_author(name="Perfil para {}".format(user.name), url=user.avatar_url)
    ...
    async def rank(self, ctx, *, user: discord.Member = None):
        ...
        # check if disabled
        if await self.config.guild(ctx.guild).disabled():
            await ctx.send("**Los comandos de niveles fueron desactivados!**")
            return
        ...
        if await self.config.guild(server).text_only():
            ...
            async with channel.typing():
                ...
                await channel.send(
                    "**Ranking & Estadisticas para {}**".format(await self._is_mention(user)),
                    file=file,
                )
            await self.db.users.update_one(
                {"user_id": str(user.id)},
    ...
    async def rank_text(self, user, server, userinfo):
        em = discord.Embed(colour=user.colour)
        em.add_field(
            name="Rango de servidor",
            value="#{}".format(await self._find_server_rank(user, server)),
        )
        em.add_field(name="Reps", value=userinfo["rep"])
        em.add_field(name="nivel de servidor", value=userinfo["servers"][str(server.id)]["level"])
        em.add_field(name="Server Exp", value=await self._find_server_exp(user, server))
        em.set_author(name="Rango & Estadisticas para {}".format(user.name), url=user.avatar_url)
        em.set_thumbnail(url=user.avatar_url)
        return em
    ...
    async def top(self, ctx, *options):
        ...
        añade el parametro -global para global y -rep para reputacion."""
        server = ctx.guild
        user = ctx.author
        ...
        async with ctx.typing():
            ...
            if "-rep" in options and "-global" in options:
                title = "Tabla de clasificacion de rep. global {}\n".format(self.bot.user.name)
                ...
                title = "Tabla de clasificacion de rep. {}\n".format(server.name)
                async for userinfo in self.db.users.find({}):
                ...
                title = "Tabla de clasificacion de exp. {}\n".format(server.name)
                async for userinfo in self.db.users.find({}):
                    try:
                        if "servers" in userinfo and str(server.id) in userinfo["servers"]:
                            server_exp = 0
            ...
            for option in options:
                if str(option).isdigit():
                    if page >= 1 and int(option) <= pages:
                        ...
                        await ctx.send(
                            "**Ingresa un numero valido! (1 - {})**".format(str(pages))
                        )
                        return
                    break
    ...
    async def rep(self, ctx, *, user: discord.Member = None):
        """Da un punto de reputación a un jugador designado."""
        org_user = ctx.author
        server = ctx.guild
        # creates user if doesn't exist
        ...
        if await self.config.guild(ctx.guild).disabled():
            await ctx.send("**Los comandos de nivel para este servidor están deshabilitados!**")
            return
        if user and user.id == org_user.id:
            await ctx.send("**No puedes darte reputación a ti mismo!**")
            return
        if user and user.bot:
            await ctx.send("**No puedes dar reputación a un bot!**")
            return
        ...
        if user and delta >= 43200.0 and delta > 0:
            ...
            await ctx.send(
                "**Acabas de dar {} puntos de reputacion!**".format(
            ...
            seconds = 43200 - delta
            if seconds < 0:
                await ctx.send("**¡Puedes dar una repetición!**")
                return

            m, s = divmod(seconds, 60)
            h, m = divmod(m, 60)
            await ctx.send(
                "**Necesitas esperar {} horas, {} minutos, y {} segundos antes de dar reputacion otra vez!**".format(
                    int(h), int(m), int(s)
                )
            )
    ...
    async def lvlinfo(self, ctx, *, user: discord.Member = None):
        """Brinda detalles más específicos sobre la imagen del perfil del usuario."""
        if not user:
            user = ctx.author
        if user.bot:
            await ctx.send_help()
        ...
        em.set_author(
            name="Informacion de perfil para {}".format(user.name),
    ...
    async def profilecolors(self, ctx, section: str, color: str):
        """Establecer color de perfil.

        Para la sección, puede elegir: `exp`,` rep`, `badge`,` info` o `all`.
        Para el color, puede utilizar: `default`,` white`, `código HEX` (# 000000) o` auto`.
        ej: `[p]lvlset profile color all #eb4034`"""
        user = ctx.author
        server = ctx.guild
        # creates user if doesn't exist
        await self._create_user(user, server)
        ...
        if section == "rep":
            ...
            await ctx.send(
                "**No es una sección válida. Debe ser `rep`,` exp`, `badge`,` info` o `all`.**"
            )
            return
        ...
        if color == "auto":
            if not all(lib in globals().keys() for lib in ["numpy", "cluster"]):
                await ctx.send("**Falta el paquete requerido. Función Autocolor no disponible**")
                return
            if section == "exp":
            ...
            await ctx.send(
                "**No es un color válido. Debe ser  `default`, `HEX color`, `white` or `auto`.**"
            )
            return
        ...
        if section == "all":
            ...
            await ctx.send("**Color para perfil {} establecido.**".format(section))

    @rankset.command(name="color")
    @commands.guild_only()
    async def rankcolors(self, ctx, section: str, color: str = None):
        """Establecer color de rango.

        Para la sección, puede elegir: `exp`,` info` o `all`.
        Para el color, puede utilizar: `default`,` white`, `código HEX` (# 000000) o` auto`.
        ej: `[p]lvlset rank color info white`"""
        user = ctx.author
        server = ctx.guild
        # creates user if doesn't exist
        await self._create_user(user, server)
        ...
        if section == "exp":
            ...
            await ctx.send("**No es una sección válida. Debe ser `exp`,` info` o `all`**")
            return
        ...
        if section == "all":
            ...
            await ctx.send("**Colores para el conjunto de rangos.**")
        else:
            await self.db.users.update_one(
                {"user_id": str(user.id)}, {"$set": {section_name: set_color[0]}}
            )
            await ctx.send("**Color para rango {} establecido.**".format(section))

    @levelupset.command(name="color")
    @commands.guild_only()
    async def levelupcolors(self, ctx, section: str, color: str = None):
        """Establecer color de subida de nivel.

        La sección solo puede ser `info`.
        El color puede ser: `default`,` white`, `código HEX` (# 000000) o` automático`.
        ej: `[p]lvlset color info default`"""
        user = ctx.author
        server = ctx.guild
        # creates user if doesn't exist
        await self._create_user(user, server)
        ...
        if section == "info":
            ...
            await ctx.send("**No es una sección válida. Debe ser `info`.**")
            return
        ...
        await ctx.send("**Color para level-up {} establecido.**".format(section))
    ...
    async def _auto_color(self, ctx, url: str, ranks):
        phrases = ["Calculando colores..."]  # in case I want more
    ...
    async def info(self, ctx, *, info):
        ...
        if len(info) < max_char:
            ...
            await ctx.send("**Tu sección de información se ha configurado correctamente!**")
        else:
            await ctx.send(
                "**¡Tu descripción tiene demasiados caracteres! Debe ser {} o menos.**".format(
                    max_char
                )
            )
    ...
    async def levelbg(self, ctx, *, image_name: str):
        """Establece tu fondo de subida de nivel."""
        user = ctx.author
        server = ctx.guild
        ...
        if image_name in backgrounds["levelup"].keys():
            if await self._process_purchase(ctx):
                ...
                await ctx.send("**Tu nuevo fondo de level-up se ha configurado correctamente!**")
        else:
            await ctx.send(
                f"Ese no es un trasfondo válido. Mira los fondos disponibles en `{ctx.clean_prefix}backgrounds levelup`."
            )

    @profileset.command(name="bg")
    @commands.guild_only()
    async def profilebg(self, ctx, *, image_name: str):
        """Establecer el fondo de su perfil."""
        user = ctx.author
        server = ctx.guild
        backgrounds = await self.config.backgrounds()
        # creates user if doesn't exist
        ...
        if image_name in backgrounds["profile"].keys():
            if await self._process_purchase(ctx):
                ...
                await ctx.send("**Su nuevo fondo de perfil se ha configurado con éxito!**")
        else:
            await ctx.send(
                f"Ese no es un trasfondo válido. Mira los fondos disponibles en `{ctx.clean_prefix}backgrounds profile`."
            )

    @rankset.command(name="bg")
    @commands.guild_only()
    async def rankbg(self, ctx, *, image_name: str):
        """Establece tu fondo de rango."""
        user = ctx.author
        server = ctx.guild
        backgrounds = await self.config.backgrounds()
        # creates user if doesn't exist
        ...
        if image_name in backgrounds["rank"].keys():
            if await self._process_purchase(ctx):
                ...
                await ctx.send("**Su nuevo fondo de pantalla de rango se ha configurado con éxito!**")
        else:
            await ctx.send(
                f"Ese no es un trasfondo válido. Mira los fondos disponibles en `{ctx.clean_prefix}backgrounds rank`."
            )
    ...
    async def title(self, ctx, *, title):
        ...
        if len(title) < max_char:
            ...
            await ctx.send("**Tu título se ha establecido con éxito!**")
        else:
            await ctx.send(
                "**¡Tu título tiene demasiados caracteres! Debe ser {} o menos.**".format(max_char)
            )
    ...
    async def toggle(self, ctx):
        """Alternar la mayoría de los comandos de niveles en el servidor actual."""
        server = ctx.guild
    ...
    async def textonly(self, ctx):
        """Alternar mensajes basados ​​en texto en el servidor."""
        server = ctx.guild
        if await self.config.guild(server).text_only():
    ...
    async def lvlalert(self, ctx):
        """Alternar mensajes de nivel en el servidor."""
        server = ctx.guild
    ...
    async def lvlprivate(self, ctx):
        """Alterna la alerta de subida de nivel en un mensaje privado para el usuario."""
        server = ctx.guild
    ...
    async def xp(self, ctx, min_xp: int = 15, max_xp: int = 20):
        """Establecer el rango de XP otorgado en cada ganancia exitosa de XP.
    ...
    async def length(self, ctx, message_length: int = 10):
        """Establecer la longitud mínima del mensaje para la ganancia de XP.
    ...
    async def available(self, ctx, badge_type: str = "server"):
        """Obtenga una lista de badges disponibles.
        ...
        if any([badge_type.casefold() == btype for btype in ["server", "guild"]]):
            ...
            await ctx.send("**Tipo de bagde no válido. Debe ser `server` o `global`.**")
            return
        em = discord.Embed(title="Badges disponibles", colour=await ctx.embed_color())
        ...
        if server_badge_info and server_badge_info["badges"]:
            ...
            for badgename in server_badges:
                ...
                msg += "**• {}** ({}) - {}\n".format(badgename, price, badgeinfo["descripcion"])
        else:
            msg = "None"
        ...
        for page in pages:
            ...
            page.set_footer(text="Pagina {}/{}".format(pagenum, len(pages)))
            pagenum += 1
    ...
    async def listuserbadges(self, ctx, user: discord.Member = None):
        ...
        for page in pagify(badge_ranks, ["\n"]):
            em.description = page
            em.set_author(name="Badges para {}".format(user.name), icon_url=user.avatar_url)
            em.set_footer(text="Pagina {} de {}".format(counter, total_pages))
            embeds.append(em)
            counter += 1
            ...
            if name in server_badges:
                ...
                if "{}_{}".format(name, str(serverid)) not in userinfo["badges"].keys():
                    ...
                    if badge_info["price"] == -1:
                        await ctx.send("**Esa insignia no se puede comprar.**".format(name))
                        ...
                        await ctx.send("**`{}` fue obteniada.**".format(name))
                    else:
                        await ctx.send(
                            "**{}, Estas a punto de comprar el badge `{}` por `{}`. Confirmar escribiendo `yes`.**".format(
                        ...
                        if badge_info["price"] <= await bank.get_balance(user):
                            ...
                            await ctx.send(
                                "**Has comprado el badge `{}` por `{}`.**".format(
                                    name, badge_info["price"]
                                )
                            )
                        elif await bank.get_balance(user) < badge_info["price"]:
                            await ctx.send(
                                "**¡Dinero insuficiente! Necesitar `{}` mas.**".format(
                                    badge_info["price"] - await bank.get_balance(user)
                                )
                            )
                else:
                    await ctx.send("**{}, ya tienes esta insignia!**".format(user.name))
            else:
                await ctx.send(
                    "**La insignia `{}` no existe. Intenta `{}badge available`**".format(
                        name, ctx.clean_prefix
                    )
                )
        else:
            await ctx.send(
                "**¡No hay insignias para conseguir! Intenta `{}badge get [badge name] -global`.**".format(
                    ctx.clean_prefix
                )
            )
    ...
    async def set_badge(self, ctx, name: str, priority_num: int):
        ...
        if priority_num < -1 or priority_num > 5000:
            await ctx.send("**¡Número de prioridad no válido! -1-5000**")
            return
        ...
        for badge in userinfo["badges"]:
            if userinfo["badges"][badge]["badge_name"] == name:
                ...
                await ctx.send(
                    "**Para el badge `{}` la prioridad se ha establecido en `{}`!**".format(
                        userinfo["badges"][badge]["badge_name"], priority_num
                    )
                )
                break
        else:
            await ctx.send("**No tienes ese badge!**")
    ...
    async def addbadge(
        ...
        *,
        ...
        `bg_img`: Indica la imagen de la insignia. (Solo se admite URL)
        `borde_color`: Indica el color del borde de la insignia. (Color HEX)
        `precio`: Indique el precio de la placa. (Indique "-1" y no se podrá comprar, "0" de forma gratuita).
        `descripcion`: Indique una descripción para su insignia.
         ej: `[p]lvlset badge add Leveler [my_url] #b60047 0 Mi super badge!`
        ...
        if await self.bot.is_owner(user):
            ...
            await ctx.send(
                "**Solo puede agregar badges en servidores con {}+ no-bot miembros**".format(
                    required_members
                )
            )
            return
        ...
        if "." in name:
            await ctx.send("**El nombre no puede contener `.`**")
            return

        if not await self._valid_image_url(bg_img):
            await ctx.send("**El fondo no es válido. Ingrese el color HEX o la URL de la imagen!**")
            return

        if not self._is_hex(border_color):
            await ctx.send("**El color del borde no es válido!**")
            return

        if price < -1:
            await ctx.send("**El precio no es válido!**")
            return

        if len(description.split(" ")) > 40:
            await ctx.send("**¡La descripción es demasiado larga! Debe tener 40 o menos.**")
            return
        ...
        if name not in badges["badges"].keys():
            ...
            await ctx.send("**`{}` Badge añadido en  `{}` server.**".format(name, servername))
        else:
            ...
            async for user in self.db.users.find({}):
                try:
                    ...
                    log.error(f"No se puede actualizar badge {name} para {user['user_id']}: {exc}")
            await ctx.send("**El badge `{}` fue actualizado**".format(name))

    @checks.is_owner()
    @badge.command()
    @commands.guild_only()
    async def type(self, ctx, name: str):
        """Defina si la insignia debe ser un círculo o barras."""
        valid_types = ["circles", "bars"]
        if name.lower() not in valid_types:
            await ctx.send("**Ese no es un tipo de credencial válido!**")
            return

        await self.config.badge_type.set(name.lower())
        await ctx.send("**Tipo de badge establecido en `{}`**".format(name.lower()))
    ...
    async def delbadge(self, ctx, *, name: str):
        """Eliminar una insignia y eliminar de todos los usuarios.
        ...
        if name in serverbadges["badges"].keys():
            ...
            async for user_info_temp in self.db.users.find({}):
                try:
                    ...
                    log.error(
                        f"No se a podido eliminar el badge {name} de {user_info_temp['user_id']}: {exc}"
                    )

            await ctx.send("**El badge `{}` fue removido.**".format(name))
        else:
            await ctx.send("**Ese badge no existe.**")
    ...
    async def give(self, ctx, user: discord.Member, name: str):
        ...
        if name not in badges:
            await ctx.send("**Ese badge no existe en este server!**")
            return
        if badge_name in badges.keys():
            await ctx.send("**{} ya tiene ese badge!**".format(await self._is_mention(user)))
            return
    ...
    async def take(self, ctx, user: discord.Member, name: str):
        """Toma la insignia de un usuario.

        Indique el usuario y el nombre de la credencial. """
        if user.bot:
            await ctx.send_help()
            return
        org_user = ctx.author
        ...
        if name not in badges:
            await ctx.send("**Ese badge no existe en este servidor!**")
        elif badge_name not in userinfo["badges"]:
            await ctx.send("**{} no tiene ese badge!**".format(await self._is_mention(user)))
        else:
            if userinfo["badges"][badge_name]["price"] == -1:
                ...
                await ctx.send(
                    "**{} ha tomado el badge `{}` de {}! :upside_down:**".format(
                ...
                await ctx.send("**No se pueden quitar las insignias que se pueden comprar!**")
    ...
    async def linkbadge(self, ctx, badge_name: str, level: int):
        ...
        if serverbadges is None:
            await ctx.send("**¡Este servidor no tiene badges!**")
            return

        if badge_name not in serverbadges["badges"].keys():
            await ctx.send("**Por favor asegúrese de que el badge`{}` existe!**".format(badge_name))
            return
        ...
        await ctx.send(
            "**El badge `{}` ha sido vinculado al nivel `{}`**".format(badge_name, level)
        )
    ...
    async def unlinkbadge(self, ctx, badge_name: str):
        """Eliminar una asociación de nivel / badge."""
        server = ctx.guild
        ...
        if badge_name in badge_links.keys():
            ...
            await ctx.send("**El bage `{}` no esta vinculado a ningun nivel!**".format(badge_name))

    @checks.mod_or_permissions(manage_roles=True)
    @badge.command(name="listlinks")
    @commands.guild_only()
    async def listbadge(self, ctx):
        """Lista de asociaciones de nivel / insignia."""
        server = ctx.guild
    ...
    async def disp_backgrounds(self, ctx, bg_type: str):
        """Obten la lista de fondos.

        puedes elegir: `profile`, `rank` o `levelup`."""
        server = ctx.guild
        backgrounds = await self.config.backgrounds()
        ...
        if bg_type.lower() == "profile":
            em.set_author(
                name="Fondos de perfil para {}".format(self.bot.user.name),
                icon_url=self.bot.user.avatar_url,
            )
            bg_key = "profile"
        elif bg_type.lower() == "rank":
            em.set_author(
                name="Fondos de rango para {}".format(self.bot.user.name),
                icon_url=self.bot.user.avatar_url,
            )
            bg_key = "rank"
        elif bg_type.lower() == "levelup":
            em.set_author(
                name="Fondos de Level-Up para {}".format(self.bot.user.name),
                icon_url=self.bot.user.avatar_url,
            )
            bg_key = "levelup"
        else:
        ...
        if bg_key:
            ...
            await ctx.send("**Tipo de fondo no válido. Debe ser `profile`, `rank` o `levelup`.**")
    ...
    async def draw_profile(self, user, server):
        ...
        draw.text(
            (await self._center(0, 140, "    RANGO", label_fnt), label_align),
            "    RANK",
            font=label_fnt,
            fill=info_text_color,
        )  # Rank
        draw.text(
            (await self._center(0, 340, "    NIVEL", label_fnt), label_align),
            "    LEVEL",
            font=label_fnt,
            fill=info_text_color,
        )  # Exp
    ...
    async def draw_rank(self, user, server):
        ...
        draw.text(
            (await self._center(100, 200, "  RANGO", label_fnt), v_label_align),
            "  RANK",
            font=label_fnt,
            fill=info_text_color,
        )  # Rank
        draw.text(
            (await self._center(100, 360, "  NIVEL", label_fnt), v_label_align),
            "  LEVEL",
            font=label_fnt,
            fill=info_text_color,
        )  # Rank
    ...
    async def _handle_levelup(self, user, userinfo, server, channel):
        ...
        if await self.config.guild(server).lvl_msg():  # if lvl msg is enabled
            if await self.config.guild(server).text_only():
                async with channel.typing():
                    em = discord.Embed(
                        description="**{} acaba de ganar un nivel{}! (NIVEL {})**".format(
